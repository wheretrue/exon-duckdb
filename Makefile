...
EXTENSION_FLAGS=-DDUCKDB_OOT_EXTENSION_NAMES="wtt01" -DDUCKDB_OOT_EXTENSION_WTT01_PATH="$(PROJ_DIR)" -DDUCKDB_OOT_EXTENSION_WTT01_SHOULD_LINK="TRUE" -DDUCKDB_OOT_EXTENSION_WTT01_INCLUDE_PATH="$(PROJ_DIR)src/include"
...
release:
	...
	cmake --build build/release --config Release -j 8 --target 'yyjson' && \
	cmake --build build/release --config Release -j 8

debug:
	mkdir -p build/debug && \
	cmake $(GENERATOR) $(FORCE_COLOR) $(EXTENSION_FLAGS) ${CLIENT_FLAGS} -DEXTENSION_STATIC_BUILD=1 -DCMAKE_BUILD_TYPE=Release ${BUILD_FLAGS} -S ./duckdb/ -B build/debug && \
...
docker-release:
	...
	docker tag public.ecr.aws/p3a4z1t3/wtt01:latest public.ecr.aws/p3a4z1t3/wtt01:v0.1.34
	docker push public.ecr.aws/p3a4z1t3/wtt01:latest
	docker push public.ecr.aws/p3a4z1t3/wtt01:v0.1.34
